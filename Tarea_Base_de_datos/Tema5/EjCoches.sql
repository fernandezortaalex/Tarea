...
SELECT personas.nombre, coches.marca from personas INNER JOIN coches ON personas.id = coches.id_dueño;
-- 2. Coches con información de dueños como su nombre y su provincia
SELECT coches.marca, personas.nombre, personas.provincia from coches INNER JOIN personas on coches.id_dueño = personas.id;
-- 3. Personas que tienen coches y su estado civil
SELECT coches.marca, personas.nombre, personas.estado_civil 
from coches 
INNER JOIN personas on coches.id_dueño = personas.id;
-- 4. Coches de personas ricas (niv socioeconomico alto)
SELECT coches.marca, personas.nivel_socioeconomico 
FROM coches
INNER JOIN personas
...
SELECT DISTINCT personas.ocupacion, coches.marca, TIMESTAMPDIFF(YEAR, personas.fecha_nacimiento, CURDATE()) as mayores_30
from personas 
INNER JOIN coches
