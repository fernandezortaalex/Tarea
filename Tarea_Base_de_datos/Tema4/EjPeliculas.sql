...
select titulo from pelicula order by anio desc;

-- 14. Calcular el promedio de duración de las películas.
select avg(duracion) as promedio_de_las_peliculas from pelicula;
-- 15. Mostrar los actores que no nacieron en EE.UU.
select * from actor where pais_origen <> 'EE.UU.';
...
select * from pelicula where anio > 2015;

-- 21. dime la mediana del año de estreno de las peliculas.
-- select avg(anio) as media_anio_estreno from pelicula;
...
SELECT pais_origen, AVG(YEAR(NOW()) - anio_nacimiento) AS edad_media
FROM actor
